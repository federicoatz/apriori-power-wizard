...
##
## The run also writes validation/monte_carlo_results.csv. This compact,
## versioned artifact records every scenario, reference method, tolerance,
## and result so readers can inspect the evidence without parsing console
## output. It is regenerated by this script, not edited by hand.
## -----------------------------------------------------------------------
...
SEED <- 20260804
RESULTS_PATH <- "validation/monte_carlo_results.csv"
validation_results <- list()

.record_result <- function(method, family, cfg_txt, formula_p, reference_p,
                           margin_2se, reps, status, interpretation) {
  validation_results[[length(validation_results) + 1L]] <<- data.frame(
    app_version = APP_VERSION,
    method = method,
    family = family,
    scenario = cfg_txt,
    formula_power = formula_p,
    reference_power = reference_p,
    difference = formula_p - reference_p,
...
.report <- function(family, cfg_txt, formula_p, sim_p, reps,
                    method = "Monte Carlo", interpretation = "Current calculation") {
  se <- sqrt(max(formula_p, sim_p) * (1 - max(formula_p, sim_p)) / reps)
  diff <- formula_p - sim_p
  flag <- if (abs(diff) <= 2 * se) "OK   " else "WATCH"
  .record_result(method, family, cfg_txt, formula_p, sim_p, 2 * se, reps,
                 trimws(flag), interpretation)
  cat(sprintf("  [%s] %-28s %-38s formula=%.4f sim=%.4f diff=%+.4f (2*SE=%.4f, reps=%d)\n",
              flag, family, cfg_txt, formula_p, sim_p, diff, 2 * se, reps))
}

.report_exact <- function(family, cfg_txt, formula_p, reference_p,
                          method = "Reference implementation") {
  diff <- formula_p - reference_p
  flag <- if (abs(diff) < 1e-6) "OK   " else "WATCH"
  .record_result(method, family, cfg_txt, formula_p, reference_p, 1e-6, NA_integer_,
                 trimws(flag), "Current calculation")
  cat(sprintf("  [%s] %-28s %-38s ours=%.6f reference=%.6f diff=%.2e\n",
              flag, family, cfg_txt, formula_p, reference_p, diff))
}
...
for (cfg in list(list(k = 3, n = 20, f = 0.25),
  ...
  .report_exact("Factorial ANOVA (1-way)",
                sprintf("k=%d n=%d f=%.2f", cfg$k, cfg$n, cfg$f),
                ours, ref, method = "pwr::pwr.anova.test()")
}
cat("\n")
...
for (cfg in list(list(m = 4,  icc = 0.30, k = 31),
  ...
  .report("Clustered (pre-1.3.0)",
          sprintf("m=%d icc=%.2f k=%d/arm d=0.50", cfg$m, cfg$icc, cfg$k),
          old, s, CLUSTER_REPS, interpretation = "Historical pre-v1.3.0 calculation")
}
cat("\n")

results_table <- do.call(rbind, validation_results)
utils::write.csv(results_table, RESULTS_PATH, row.names = FALSE, na = "")
cat(sprintf("Wrote %d validation rows to %s\n", nrow(results_table), RESULTS_PATH))
